...
export default function PositionCard({ position, onClose }: PositionCardProps) {
  ...
  const pnlPercentage = ((position.current_pnl / (position.entry_price * position.quantity)) * 100).toFixed(2);
      ...
      <CardContent className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-semibold">${position.current_price?.toLocaleString() ?? 'N/A'}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-semibold">
              ${((position.current_price ?? 0) * position.quantity).toLocaleString()}
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="text-right">
                <p className={`text-lg font-bold ${isProfit ? 'text-green-600' : 'text-red-600'}`}>
                  ${Math.abs(position.current_pnl).toLocaleString()}
                </p>
